...
function transformations_addPositions(project, options = {}) {
	...
	for(let [m,model] of Object.entries(project.model || {})){
		...
		let {$errors, ...modelPositions} = recurseModels(
			model, [`model`,m], filePaths, positions.file, trace, 0, 0, fastPositions
			)
...
function recurseModels(context, valuePath, modelFiles=[], filePositions, trace, d=0, lastMatchedFileIdx=0, fastPositions=false){
	...
	if(typeof context === "object" && !positionsNotFound){
		if (Array.isArray(context) && context.length === 1 && typeof context[0] !== 'object') {
			let refPositions = recurseModels(context[0], valuePath, modelFiles, filePositions, trace, d+1, lastMatchedFileIdx, fastPositions)
		...
		if (!fastPositions || valuePath.length < 4) {
			for(let [k,val] of Object.entries(context)){
				...
				let refPositions = recurseModels(
					val, [...valuePath, k], modelFiles, filePositions, trace, d+1, lastMatchedFileIdx, fastPositions
					)
