...
function transformAst(astNode, options = {}) {
	...
	if (astNode.type === 'lookml' || astNode.type === 'file') {
		...
		const { collection, $strings } = groupDeclarations(declarations, opts, baseLine)
...
function transformDeclaration(declNode, options = {}) {
	...
	if (declNode.type === 'object') {
		...
		const res = transformAst(declNode.value, { ...options, _extraDecls: extraInnerDecls, _baseLine: baseLine })
			...
			if (fragmentText.trim().length) {
				try {
					...
					if (e && e.location && e.location.start) {
						...
						if (fragLine === 1) {
							e.location.start.column = (e.location.start.column || 1) + colOffset
							e.location.end.column = (e.location.end.column || 1) + colOffset
						} else {
