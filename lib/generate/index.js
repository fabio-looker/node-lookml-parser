...
function generateLookmlArray(projectFragment,{indentation=""}={}){
	...
	for(let string of strings){
		if(isSegment(string)){
			...
			const substrings = segment.length > 1 ? segment.slice(1) : [["!"]]
			flatPush(out, substrings.map(str => 
				!isSelfReference(str) ? [str] 
				: typeof value === "string" ? [value]
				: generateLookmlArray(value,nextIndentation)
				)
				.reduce(flatten,[])
				)
			continue
...
function isSelfReference(x){return Array.isArray(x) && x[0] && x[0]==='!'}
