...
function generateNode(stringsNode, dataVal, parentKey, options = {}) {
	...
	if (Array.isArray(stringsNode)) {
		resultText = formatLeafTokens(stringsNode, dataVal, parentKey)
		...
		if (Array.isArray(seq)) {
			...
			while (i < closingIdx) {
				...
				if (Array.isArray(item)) {
					...
					if (key === '$value') {
						out.push(String(dataVal !== undefined ? dataVal : ''))
						i++
						continue
					}
...
function formatLeafTokens(tokens, dataVal, parentKey) {
	...
	const typeStr = parentKey ? String(parentKey).replace(/^\$/, '') : ''

	const resultTokens = tokens.map(t => {
		if (typeof t === 'string') return t
		if (Array.isArray(t)) {
			if (t[0] === '$value') return valStr
			if (t[0] === '$type') return typeStr
